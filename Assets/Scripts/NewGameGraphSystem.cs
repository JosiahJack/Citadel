...
// Duplicate of BiomonitorGraph but faked out...aaand I redesigned it entirely!
public class NewGameGraphSystem : MonoBehaviour {
    ...
    void Start() {
        min = new float[]{-1f,-2f,-1f};
        max = new float[]{ 1f, 2f, 1f};
        graphWidth = (int)((float)Screen.width * widthPerc * 0.6f);
        graphHeight = (int)((float)Screen.height * heightPerc * 0.7f);
            ...
            case 0:
                ...
                if (Mathf.Abs(lastERG - y0) > 2) {
                    ...
                    while (dist != y0) {
                        ...
                        colorsERG[currentIndex1][dist] = ergColor;
                        if (down) dist--;
                        else dist++;
                    }
                }
