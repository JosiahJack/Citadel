...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScript : MonoBehaviour {
        ...
        private void GenerateLightningBolt(Vector3 start, Vector3 end, int generation, int totalGenerations, float offsetAmount) {
            if (generation < 0 || generation > 8) return;

            //if (orthographic) start.z = end.z = Mathf.Min(start.z, end.z);
        ...
        private void SelectOffsetFromAnimationMode() {
            if (AnimationMode == LightningBoltAnimationMode.None) { lineRenderer.material.mainTextureOffset = offsets[0]; return; }
            ...
            } else index = RandomGenerator.Next(0, offsets.Length);

            if (index >= 0 && index < offsets.Length)lineRenderer.material.mainTextureOffset = offsets[index];
            else  lineRenderer.material.mainTextureOffset = offsets[0];
        }
