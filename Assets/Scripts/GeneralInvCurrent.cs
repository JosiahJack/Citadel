...
public class GeneralInvCurrent : MonoBehaviour {
	public int generalInvCurrent = new int();
	public int generalInvIndex = new int();
	...
	void Update() {
		...
		if (setVaporizeButtonOn) vaporizeButton.SetActive(true);
		else vaporizeButton.SetActive(false);

		if (setActivateButtonOn) activateButton.SetActive(true);
